...
# Loss functions for segmentation from Topcoders DSBowl2018
...
def dice_coef(y_true, y_pred, smooth=1e-3):
    ...
    intersection = K.sum(y_true_f * y_pred_f)
    return K.mean(
        (2.0 * intersection + smooth) / (K.sum(y_true_f) + K.sum(y_pred_f) + smooth)
    )
...
def dice_coef_loss_bce(y_true, y_pred, dice=0.5, bce=0.5):
    return (
        binary_crossentropy(y_true, y_pred) * bce
...
def softmax_dice_loss(y_true, y_pred):
    return (
        categorical_crossentropy(y_true, y_pred) * 0.6
