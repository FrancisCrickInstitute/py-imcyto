...
def probability_basin_watershed_2(
    nuc_im, boundary_im, com_im, thresh_nuc, thresh_com, min_obj_size=4, compactness=10
):
    """
    ...
    prob_boundary_im = np.divide(inverted_boundary_im, 255.0)

    # do the same for the whole nucleus image
    prob_nuc_im = np.divide(nuc_im, 255.0)
...
def instance_closing(seg_mask, strel=None, min_size=5):
    ...
    seg_mask = cp.pad(
        seg_mask, (image_pad, image_pad), mode="constant", constant_values=0
    )
    ...
    if len(predict_labels > 0):
        ...
        for i, lbl in tqdm(enumerate(predict_labels), ascii=True):
            ...
            pad_instance = cp.pad(
                instance, (instance_pad, instance_pad), "constant", constant_values=0
            )
            ...
            instance_closed = remove_small_objects(
                instance_closed, min_size=min_size, connectivity=1
            )
            instance_closed = remove_small_holes(
                instance_closed, area_threshold=min_size, connectivity=1
            )
